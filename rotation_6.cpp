...
// Rotation about arbitrary (rx,ry)
void rotation(double ang,int rx,int ry){
    double rad=ang*M_PI/180.0;
    for(int i=0;i<n;i++){
        int x=m[i][0]-rx, y=m[i][1]-ry;
        int xr = (int)( x*cos(rad) - y*sin(rad) ) + rx;
        int yr = (int)( x*sin(rad) + y*cos(rad) ) + ry;
        m[i][0]=xr; m[i][1]=yr;
    }
    glColor3f(1,1,0);
    for(int i=0;i<n;i++){
...
void menu(int c){
    if(c==1) choice();
        ...
        int rx,ry; double ang;
        cout<<"pivot x y? ";cin>>rx>>ry;
        cout<<"angle? ";cin>>ang;
        rotation(ang,rx,ry);
    }
    else exit(0);
...
int main(int argc,char**argv){
    ...
    glutCreateWindow("Translate + Rotate");
    init();
